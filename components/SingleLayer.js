...
function updateSingleLayerMap (showLayerWithBreaks, root, op, layer, t1) {
...
var SingleLayer = React.createClass({
  ...
  updateMap: function (state) {
    ...
    ifAllDefined(this.props.showLayerWithBreaks, this.props.rootUrl, state.operation, this.props.layers[state.layerId], state.timeId)
  ...
  /** Use this as an opportunity to react to a prop transition before render() is called by updating the state using this.setState().
    * The old props can be accessed via this.props. Calling this.setState() within this function will not trigger an additional render. */
    if ( _.isUndefined(this.state.layerId) && ! _.isEmpty(nextProps.layers)) {
      ...
      updateSingleLayerMap(nextProps.showLayerWithBreaks, nextProps.rootUrl, this.state.operation, layer, 0);
